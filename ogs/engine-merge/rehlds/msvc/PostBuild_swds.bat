...
IF NOT EXIST "%projectDir%\PublishPath_swds.txt" (
	ECHO 	No deployment path specified. Create PublishPath_swds.txt near PostBuild_swds.bat with paths on separate lines for auto deployment.
	exit /B 0
)

FOR /f "tokens=* delims= usebackq" %%a IN ("%projectDir%\PublishPath_swds.txt") DO (
	ECHO Deploying to: %%a
	IF NOT "%%a" == "" (
