   ...
   fdiv_bug and fpu emulation flags -- alex/MPlayer */
#ifdef __linux__
	{
		 ...
		 */
			...
			if((!lstrncmpiA(line, "flags", strlen("flags"))) || (!lstrncmpiA(line, "features", strlen("features"))))
			{
				if(strstr(value, "cx8"))
...
static long WINAPI expHeapFree(HANDLE heap, DWORD dwFlags, LPVOID lpMem)
{
	...
	if(heapfreehack != lpMem && lpMem != (void *)0xffffffff && lpMem != (void *)0xbdbdbdbd)
	...
	if(f != NULL)
	{
		while(fgets(line, sizeof(line), f) != NULL)
		{
			...
			*value++ = '\0';
			...
			if(!strncasecmp(line, "cpu MHz", strlen("cpu MHz")) && sscanf(value, "%lf", &freq) == 1)
			{
				freq *= 1000;
				break;
			}
...
static void *WINAPI expGetProcAddress(HMODULE mod, char *name)
{
	...
	case MODULE_HANDLE_comdlg32:
		result = LookupExternalByName("comdlg32.dll", name);
		break;
	case MODULE_HANDLE_msvcrt:
		result = LookupExternalByName("msvcrt.dll", name);
		break;
	case MODULE_HANDLE_ole32:
		result = LookupExternalByName("ole32.dll", name);
		break;
	case MODULE_HANDLE_winmm:
		result = LookupExternalByName("winmm.dll", name);
		break;
	case MODULE_HANDLE_psapi:
		result = LookupExternalByName("psapi.dll", name);
		break;
...
#define FPU_DOUBLES(var1, var2)       \
	double var1, var2;                \
	__asm__ volatile("fstpl %0;fwait" \
	                 : "=m"(var2)     \
	                 :);              \
	__asm__ volatile("fstpl %0;fwait" \
	                 : "=m"(var1)     \
