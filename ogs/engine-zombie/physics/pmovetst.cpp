...
pmtrace_t _PM_PlayerTrace(vec_t *start, vec_t *end, int traceFlags, int numphysent, physent_t *physents, int ignore_pe, int (*pfnIgnore)(physent_t *))
{
	...
	for(int i = 0; i < numphysent; i++)
	{
		...
		if((pe->model && !pe->solid && pe->skin) ||
		   ((traceFlags & PM_GLASS_IGNORE) && pe->rendermode))
			continue;
		...
		else
		{
			...
			if(pe->studiomodel)
			{
				    ...
				    (pmove->usehull == 2 && !(traceFlags & PM_STUDIO_BOX))))
				{
					...
					pe->studiomodel, offset, pe->frame, pe->sequence, pe->angles, pe->origin, pe->controller, pe->blending, &pNumHulls);
				}
			}
		...
		if(pe->solid == SOLID_BSP &&
		   (pe->angles[0] != 0.0 || pe->angles[1] != 0.0 ||
		    pe->angles[2] != 0.0))
		{
			vec3_t forward, right, up;
			AngleVectors(pe->angles, forward, right, up);
		...
		else
		{
			...
			else
			{
				...
				for(int j = 0; j < pNumHulls; j++)
				{
					...
					if(j == 0 || testtrace.allsolid || testtrace.startsolid ||
					   testtrace.fraction < total.fraction)
					{
