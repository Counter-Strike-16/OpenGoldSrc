...
{
	FileHandle_t file = gpFileSystem->Open(pstFileName, "rb");

	if(file == FILESYSTEM_INVALID_HANDLE)
		return FALSE;

	BlobInfo_t info;
	gpFileSystem->Read(&info, sizeof(BlobInfo_t), file);
...
DWORD NLoadBlobFile(const char *pstFileName, BlobFootprint_t *pblobfootprint, void **pv)
{
	...
	gpFileSystem->Seek(file, 0, FILESYSTEM_SEEK_TAIL);
	dwSize = gpFileSystem->Tell(file);
	gpFileSystem->Seek(file, 0, FILESYSTEM_SEEK_HEAD);

	pBuffer = (BYTE *)malloc(dwSize);
	gpFileSystem->Read(pBuffer, dwSize, file);
