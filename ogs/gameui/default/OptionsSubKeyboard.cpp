...
void COptionsSubKeyboard::OnPageShow(void)
{
}
...
void COptionsSubKeyboard::OnThink(void)
{
	BaseClass::OnThink();
}
...
public:
	    ...
	    : BaseClass(NULL, NULL)
	{
		...
		SetSize(280, 140);
		LoadControlSettingsFromScheme("OptionsSubKeyboardAdvancedDlg.res");
	...
	virtual void Activate(void)
	{
		...
		SetControlInt("ConsoleCheck", g_pcvarEnableConsole->value ? 1 : 0);
		SetControlInt("FastSwitchCheck", engine->pfnGetCvarFloat("hud_fastswitch") ? 1 : 0);
	}

	virtual void OnApplyData(void)
	{
		engine->Cvar_SetValue("bte_enableconsole", GetControlInt("ConsoleCheck", 0));
		engine->Cvar_SetValue("hud_fastswitch", GetControlInt("FastSwitchCheck", 0));
	}
