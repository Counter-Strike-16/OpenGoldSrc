...
void Host_UpdateStats()
{
	...
	if(last + 1.0 < Sys_FloatTime())
	{
		...
		cpuPercent =
		    (FILETIME_TO_QWORD(UserTime) + FILETIME_TO_QWORD(KernelTime) - lastrunticks) / (FILETIME_TO_QWORD(SystemTimeAsFileTime) - (double)FILETIME_TO_PAIR(runticks, cputicks));
...
int Host_Load(const char *pName)
{
	...
	if(!Q_stricmp(com_gamedir, "valve") || !Q_stricmp(com_gamedir, "bshift") || !Q_stricmp(com_gamedir, "gearbox"))
	{
		g_psvs.maxclients = 1;
