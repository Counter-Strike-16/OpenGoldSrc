...
void Host_UpdateStats()
{
	...
	if(last + 1.0 < Sys_FloatTime())
	{
		...
		if(lastAvg + 5.0f < Sys_FloatTime())
		{
			...
			lastrunticks =
			FILETIME_TO_QWORD(UserTime) + FILETIME_TO_QWORD(KernelTime);
			lastAvg = last;
		}
		last = Sys_FloatTime();
	}
	...
	if(Sys_FloatTime() > last + 1.0f)
	{
		...
		if(lastrunticks)
			cpuPercent =
			(double)(cputicks - lastcputicks) / (double)(runticks - lastrunticks);
		else
...
{
#ifdef HOOK_ENGINE
	Cmd_AddCommand("shutdownserver",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_KillServer_f", (void *)Host_KillServer_f));
	Cmd_AddCommand("soundfade",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_Soundfade_f", (void *)Host_Soundfade_f));
	Cmd_AddCommand("status", (xcommand_t)GetOriginalFuncAddrOrDefault("Host_Status_f", (void *)Host_Status_f));
	Cmd_AddCommand(
	"stat", (xcommand_t)GetOriginalFuncAddrOrDefault("Host_Status_Formatted_f", (void *)Host_Status_Formatted_f));
	Cmd_AddCommand("quit", (xcommand_t)GetOriginalFuncAddrOrDefault("Host_Quit_f", (void *)Host_Quit_f));
	Cmd_AddCommand("_restart",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_Quit_Restart_f", (void *)Host_Quit_Restart_f));
	...
	Cmd_AddCommand("changelevel",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_Changelevel_f", (void *)Host_Changelevel_f));
	Cmd_AddCommand("changelevel2",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_Changelevel2_f", (void *)Host_Changelevel2_f));
	Cmd_AddCommand("reconnect",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_Reconnect_f", (void *)Host_Reconnect_f));
	...
	Cmd_AddCommand("pause",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_TogglePause_f", (void *)Host_TogglePause_f));
	...
	Cmd_AddCommand("motd_write",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_Motd_Write_f", (void *)Host_Motd_Write_f));
	...
	Cmd_AddCommand("writecfg",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_WriteCustomConfig", (void *)Host_WriteCustomConfig));
	Cmd_AddCommand("startdemos",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Host_Startdemos_f", (void *)Host_Startdemos_f));
	...
	Cmd_AddCommand("setmaster",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Master_SetMaster_f", (void *)Master_SetMaster_f));
	Cmd_AddCommand("heartbeat",
	               (xcommand_t)GetOriginalFuncAddrOrDefault(
	               "Master_Heartbeat_f", (void *)Master_Heartbeat_f));
#else // HOOK_ENGINE
