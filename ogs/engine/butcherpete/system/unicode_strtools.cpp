...
{
	// Values > 0x10FFFF are explicitly invalid; ditto for UTF-16 surrogate
	// halves,
	// values ending in FFFE or FFFF, or values in the 0x00FDD0-0x00FDEF reserved
	// range
	return (uVal < 0x110000u) && ((uVal - 0x00D800u) > 0x7FFu) &&
	((uVal & 0xFFFFu) < 0xFFFEu) && ((uVal - 0x00FDD0u) > 0x1Fu);
}

// Decode one character from a UTF-8 encoded string. Treats 6-byte CESU-8
// sequences
// as a single character, as if they were a correctly-encoded 4-byte UTF-8
// sequence.
int Q_UTF8ToUChar32(const char *pUTF8_, uchar32 &uValueOut, bool &bErrorOut)
{
	...
	// Expecting at least a two-byte sequence with 0xC0 <= first <= 0xF7
	// (110...... and 11110...)
...
decodeFinishedMaybeCESU8:
	// Do we have a full UTF-16 surrogate pair that's been UTF-8 encoded
	// afterwards?
	// That is, do we have 0xD800-0xDBFF followed by 0xDC00-0xDFFF? If so, decode
	// it all.
	if((uValue - 0xD800u) < 0x400u && pUTF8[3] == 0xED &&
	   (uint8)(pUTF8[4] - 0xB0) < 0x10 && (pUTF8[5] & 0xC0) == 0x80)
	{
		uValue = 0x10000 + ((uValue - 0xD800u) << 10) +
		((uint8)(pUTF8[4] - 0xB0) << 6) + pUTF8[5] - 0x80;
		nBytes = 6;
		uMinValue = 0x10000;
	}
	goto decodeFinished;
...
//			if it is technically a space or punctuation, but causes
//disruptive
//			behavior when used in names, web pages, chat windows,
//etc.
//
//			characters in this set are removed from the beginning and/or
//end of strings
...
qboolean Q_UnicodeValidate(const char *pUTF8)
{
	...
	{
		uchar32 uVal;
		// Our UTF-8 decoder silently fixes up 6-byte CESU-8 (improperly re-encoded
		// UTF-16) sequences.
		// However, these are technically not valid UTF-8. So if we eat 6 bytes at
		// once, it's an error.
...
template <typename T_IN, typename T_OUT, bool UNK, qboolean (*IN_TO_UCHAR32)(const T_IN *pUTF8, uchar32 &uValueOut, bool &bErrorOut), int(UCHAR32_TO_OUT_LEN)(uchar32 uVal), int(UCHAR32_TO_OUT)(uchar32 uVal, T_OUT *pUTF8Out)>
...
int Q_UTF8ToUTF16(const char *pUTF8, uchar16 *pUTF16, int cubDestSizeInBytes, enum EStringConvertErrorPolicy ePolicy)
{
	...
	pUTF8, pUTF16, cubDestSizeInBytes, ePolicy);
}
...
qboolean Q_StripUnprintableAndSpace(char *pch)
{
	...
	int cwch = (unsigned int)Q_UTF8ToUTF16(pch, (uchar16 *)pwch_alloced, cubDest, _STRINGCONVERTFLAG_ASSERT) >>
	1;
