...
void Mod_ClearAll()
{
	...
	for(i = 0, mod = mod_known; i < mod_numknown; i++, mod++)
	{
		if(mod->type != mod_alias &&
		   mod->needload != (NL_NEEDS_LOADED | NL_UNREFERENCED))
		{
	...
	if(trackCRC)
	{
		...
		if(p->shouldCRC)
		{
			...
			if(p->firstCRCDone)
			{
				...
				{
					Sys_Error("%s has been modified since starting the engine.  Consider "
					          "running system diagnostics to check for faulty "
					          "hardware.\n",
					          mod->name);
				}
			}
...
void Mod_LoadTextures(lump_t *l)
{
	...
	loadmodel->textures =
	(texture_t **)Hunk_AllocName(4 * loadmodel->numtextures, loadname);
	...
	for(int i = 0; i < m->_nummiptex; i++)
	{
		...
		tx = (texture_t *)Hunk_AllocName(
		2 + 8 * palette + pixels + sizeof(texture_t), loadname);
...
void Mod_LoadFaces(lump_t *l)
{
	...
	for(surfnum = 0; surfnum < count; surfnum++, in++, out++)
	{
		...
		if(texName[0] == '!' || !Q_strnicmp(texName, "laser", 5) ||
		   !Q_strnicmp(texName, "water", 5)) // turbulent
		{
...
void *Mod_LoadAliasGroup(void *pin, int *pframeindex, int numv, trivertx_t *pbboxmin, trivertx_t *pbboxmax, aliashdr_t *pheader, char *name)
{
	...
	paliasgroup = (maliasgroup_t *)Hunk_AllocName(
	sizeof(paliasgroup->frames[0]) * (numframes - 1) + sizeof(maliasgroup_t),
	loadname);
...
void *Mod_LoadAliasSkinGroup(void *pin, int *pskinindex, int skinsize, aliashdr_t *pheader)
{
	...
	paliasskingroup = (maliasskingroup_t *)Hunk_AllocName(
	...
	*pskinindex = (byte *)paliasskingroup - (byte *)pheader;
	poutskinintervals =
	(float *)Hunk_AllocName(sizeof(float) * numskins, loadname);
...
void Mod_LoadAliasModel(model_t *mod, void *buffer)
{
	...
	sizeof(stvert_t) * LittleLong(pinmodel->numverts) + sizeof(mdl_t) +
	sizeof(aliashdr_t) +
	...
	pskindesc = (maliasskindesc_t *)Hunk_AllocName(
	sizeof(maliasskindesc_t) * numskins, loadname);
	...
	for(i = 0; i < numframes; i++)
	{
		...
		if(frametype == ALIAS_SINGLE)
			...
			pframetype + 1, &pheader->frames[i].frame, pmodel->numverts, &pheader->frames[i].bboxmin, &pheader->frames[i].bboxmax, pheader, pheader->frames[i].name);
		else
	...
	PackedColorVec *pPal =
	(PackedColorVec *)Hunk_AllocName(sizeof(PackedColorVec) * 256, loadname);
...
void Mod_LoadSpriteModel(model_t *mod, void *buffer)
{
	...
	size = sizeof(msprite_t) + (numframes - 1) * sizeof(psprite->frames) + 2 +
	8 * palsize;
	...
	(unsigned char *)(&pin[1]) + 2; // header + palette size
	uint16 *paldest = (uint16 *)((char *)(psprite + 1) +
	                             sizeof(psprite->frames) *
	                             (numframes - 1)); // sprite + [frames-1]
