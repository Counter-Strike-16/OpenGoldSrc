	...
	for(int i = 0; i < delta->fieldCount; i++)
	{
		...
		if(fieldDesc->fieldType != DT_STRING)
		{
			...
			while(blockStart < fieldDesc->fieldOffset + fieldSize)
			{
				...
				uint32 mask = DELTAJIT_CreateMask(fieldDesc->fieldOffset - blockStart,
				                                  fieldDesc->fieldOffset + fieldSize -
				                                  blockStart);
...
public:
	...
	delta_marked_mask_t originalMarkedFieldsMask; // mask based on data, before
...
public:
	...
	void checkFieldMask(jitasm::Frontend::Reg32 &mask,
	                    deltajit_memblock_field *jitField);
...
void CDeltaCheckJIT::checkFieldMask(jitasm::Frontend::Reg32 &mask,
                                    deltajit_memblock_field *jitField)
{
...
void CDeltaCheckJIT::iterateBlocks(deltajitdata_t *jitdesc)
{
#ifndef REHLDS_FIXES
	sub(esp,
	    12); // some local storage is required for precise DT_TIMEWINDOW marking
#endif
  ...
  */
	...
	{
		movdqu(src_xmm[0],
		       xmmword_ptr[esi + (jitdesc->itrBlocks[0].memblockId * 16)]);
		movdqu(dst_xmm[0],
		       xmmword_ptr[edi + (jitdesc->itrBlocks[0].memblockId * 16)]);
	}
	if(jitdesc->numItrBlocks > 1)
	{
		movdqu(src_xmm[1],
		       xmmword_ptr[esi + (jitdesc->itrBlocks[1].memblockId * 16)]);
		movdqu(dst_xmm[1],
		       xmmword_ptr[edi + (jitdesc->itrBlocks[1].memblockId * 16)]);
	}
	if(jitdesc->numItrBlocks > 2)
	{
		movdqu(src_xmm[2],
		       xmmword_ptr[esi + (jitdesc->itrBlocks[2].memblockId * 16)]);
		movdqu(dst_xmm[2],
		       xmmword_ptr[edi + (jitdesc->itrBlocks[2].memblockId * 16)]);
	}
	...
	for(unsigned int i = 0; i < jitdesc->numItrBlocks; i++)
	{
		...
		pcmpeqb(src_xmm[dataXmmCounter], dst_xmm[dataXmmCounter]); // 0..15: byte[s]
		                                                           // = (byte[s] ==
		                                                           // byte[d]) ?
		                                                           // 0xFF : 0x00
		...
		{
			movdqu(src_xmm[dataXmmCounter],
			       xmmword_ptr[esi + (jitdesc->itrBlocks[i + 3].memblockId * 16)]);
			movdqu(dst_xmm[dataXmmCounter],
			       xmmword_ptr[edi + (jitdesc->itrBlocks[i + 3].memblockId * 16)]);
		}
		...
		for(unsigned int j = 0; j < block->numFields; j++)
		{
			...
			if(jitField->field->type == DT_TIMEWINDOW_8 ||
			   jitField->field->type == DT_TIMEWINDOW_BIG)
			{
				if(jitField->last)
				{
					float multiplier =
					(jitField->field->type == DT_TIMEWINDOW_8) ? 100.0f : 1000.0f;
...
class CDeltaClearMarkFieldsJIT
: public jitasm::function<int, CDeltaClearMarkFieldsJIT, void *, void *, void *, void *>
{
public:
...
CDeltaClearMarkFieldsJIT::CDeltaClearMarkFieldsJIT(deltajitdata_t *_jitdesc)
    : jitdesc(_jitdesc)
{
}
...
CDeltaClearMarkFieldsJIT::Result
CDeltaClearMarkFieldsJIT::main(Addr src, Addr dst, Addr deltaJit, Addr pForceMarkMask)
{
	/*
          Registers usage:
                  esi = src
  ...
  */
	...
	// Save mask from SSE register to CDeltaJit::marked_fields_mask and
	// CDeltaJit::originalMarkedFieldsMask
	mov(ebx, ptr[deltaJit]);
	movq(qword_ptr[ebx + delta_markbits_offset], marked_fields_mask);
	movq(qword_ptr[ebx + offsetof(CDeltaJit, originalMarkedFieldsMask)],
	     marked_fields_mask);
...
class CDeltaTestDeltaJIT
: public jitasm::function<int, CDeltaTestDeltaJIT, void *, void *, void *>
{
public:
...
CDeltaTestDeltaJIT::CDeltaTestDeltaJIT(deltajitdata_t *_jitdesc)
    : jitdesc(_jitdesc)
{
}
...
void CDeltaTestDeltaJIT::onStringChecked(deltajit_field *field)
{
	...
	lea(neededBits,
	    ptr[neededBits + eax * 8 + 8]); // add size of string in bits + EOS byte
...
CDeltaJit::CDeltaJit(delta_t *_delta,
                     CDeltaClearMarkFieldsJIT *_cleanMarkCheckFunc,
                     CDeltaTestDeltaJIT *_testDeltaFunc)
{
	delta = _delta;
...
void CDeltaJitRegistry::CreateAndRegisterDeltaJIT(delta_t *delta)
{
	...
	CDeltaClearMarkFieldsJIT *cleanMarkCheckFunc =
	new CDeltaClearMarkFieldsJIT(&data);
...
NOINLINE int DELTAJit_Fields_Clear_Mark_Check(unsigned char *from,
                                              unsigned char *to,
                                              delta_t *pFields,
                                              void *pForceMarkMask)
{
...
{
	CDeltaJit *deltaJit = DELTAJit_LookupDeltaJit(__FUNCTION__, pFields);
	deltaJit->marked_fields_mask.u32[fieldNumber >> 5] |=
	(1 << (fieldNumber & 31));
}
...
{
	CDeltaJit *deltaJit = DELTAJit_LookupDeltaJit(__FUNCTION__, pFields);
	return deltaJit->marked_fields_mask.u32[fieldNumber >> 5] &
	(1 << (fieldNumber & 31));
}
