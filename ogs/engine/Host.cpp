...
cvar_t skill = {"skill", "1", 0, 0.0f, NULL}; // 0 - 3
cvar_t deathmatch = {"deathmatch", "0", FCVAR_SERVER, 0.0f, NULL}; // 0, 1, or 2
cvar_t coop = {"coop", "0", FCVAR_SERVER, 0.0f, NULL}; // 0 or 1
...
void CHost::Shutdown()
{
	...
	if(mpLocalClient->GetState() != ca_dedicated) // host_basepal instead of this in qw
		VID_Shutdown();
};
...
// update audio
	...
	{
		mpSound->Update(r_origin, vpn, vright, vup);
		CL_DecayLights();
	}
	else
		mpSound->Update(vec3_origin, vec3_origin, vec3_origin, vec3_origin);
