...
// fix CreateFileMappingA
	...
	if(nt->FileHeader.Machine != IMAGE_FILE_MACHINE_I386)
	{
		...
		case IMAGE_FILE_MACHINE_POWERPC:
			...
			break;
		default:
			dbg_printf("Unknown-%04x", nt->FileHeader.Machine);
			break;
		}
		dbg_printf(")\n");
		goto error;
	   ...
	   !strstr(filename, "xanlib.dll"))
	{
		...
		if(!IsBadReadPtr((LPBYTE)sharedMod, sizeof(IMAGE_DOS_HEADER)) && memcmp((LPBYTE)sharedMod, (LPBYTE)hModule, sizeof(IMAGE_DOS_HEADER)) == 0 && !IsBadReadPtr(sharedNt, sizeof(IMAGE_NT_HEADERS)) && memcmp(sharedNt, nt, sizeof(IMAGE_NT_HEADERS)) == 0)
		{
	...
	if(load_addr == 0)
	{
		...
		else
		{
			FIXME("FATAL: Need to relocate %s, but no relocation records present (%s). Try to run that file directly !\n",
			      filename,
			      (nt->FileHeader.Characteristics & IMAGE_FILE_RELOCS_STRIPPED) ? "stripped during link" : "unknown reason");
			goto error;
		}
...
/*
	...
	if(pe_import && !(wm->flags & WINE_MODREF_LOAD_AS_DATAFILE) && !(wm->flags & WINE_MODREF_DONT_RESOLVE_REFS) && fixup_imports(wm))
	{
